...
export const supabaseServices: AdminServices = {
  ...
  marketplace: {
    ...
    async listDrivers(projectId) { const { data,error }=await getSupabaseClient().rpc("admin_list_marketplace_drivers",{target_project_id:projectId}); throwIfError(error); return ((data??[]) as Record<string,unknown>[]).map(r=>({userId:String(r.user_id),displayName:String(r.display_name),phone:r.phone?String(r.phone):null,status:String(r.status),trialActive:Boolean(r.trial_active),vehicleName:r.vehicle_name?String(r.vehicle_name):null,isAvailable:r.is_available==null?null:Boolean(r.is_available),walletAvailableBalance:r.wallet_available_balance==null?null:Number(r.wallet_available_balance)} satisfies MarketplaceDriver)); },
    async listJobs(projectId, filters={}) { const { data,error }=await getSupabaseClient().rpc("admin_list_marketplace_jobs",{target_project_id:projectId,target_status:filters.status??null,target_service_code:filters.serviceCode??null}); throwIfError(error); return ((data??[]) as Record<string,unknown>[]).map(r=>({jobId:String(r.job_id),status:String(r.status),serviceCode:String(r.service_code),originText:String(r.origin_text),destinationText:String(r.destination_text),finalPrice:Number(r.final_price),currency:String(r.currency),customerDisplayName:r.customer_display_name?String(r.customer_display_name):null,driverDisplayName:r.driver_display_name?String(r.driver_display_name):null,vehicleName:r.vehicle_name?String(r.vehicle_name):null,billingMode:r.billing_mode?String(r.billing_mode):null,incidentReason:r.incident_reason?String(r.incident_reason):null,createdAt:String(r.created_at)} satisfies MarketplaceJob)); },
    ...
    async listCustomers(projectId) { const {data,error}=await getSupabaseClient().rpc("admin_list_marketplace_customers",{target_project_id:projectId}); throwIfError(error); return ((data??[]) as Record<string,unknown>[]).map(r=>({customerId:String(r.customer_id),displayName:String(r.display_name),whatsappPhone:String(r.whatsapp_phone),jobsTotal:Number(r.jobs_total),jobsActive:Number(r.jobs_active),jobsSettled:Number(r.jobs_settled),lastJobAt:r.last_job_at?String(r.last_job_at):null} satisfies MarketplaceCustomer)); },
    async listTopups(projectId,status) { const {data,error}=await getSupabaseClient().rpc("admin_list_marketplace_topups",{target_project_id:projectId,target_status:status??null}); throwIfError(error); return ((data??[]) as Record<string,unknown>[]).map(r=>({topupId:String(r.topup_id),userId:String(r.user_id),driverDisplayName:String(r.driver_display_name),amount:Number(r.amount),currency:String(r.currency),status:String(r.status),method:String(r.method),reference:r.reference?String(r.reference):null,requestedAt:String(r.requested_at)} satisfies MarketplaceTopup)); },
    async listWallets(projectId) { const {data,error}=await getSupabaseClient().rpc("admin_list_marketplace_wallets",{target_project_id:projectId}); throwIfError(error); return ((data??[]) as Record<string,unknown>[]).map(r=>({userId:String(r.user_id),driverDisplayName:String(r.driver_display_name),currency:String(r.currency),totalBalance:Number(r.total_balance),reservedBalance:Number(r.reserved_balance),availableBalance:Number(r.available_balance),initialDepositConfirmed:Boolean(r.initial_deposit_confirmed),updatedAt:String(r.updated_at)} satisfies MarketplaceWallet)); },
    async financialSettings(projectId) { const {data,error}=await getSupabaseClient().rpc("admin_get_marketplace_financial_settings",{target_project_id:projectId}); throwIfError(error); const r=((data as Record<string,unknown>[])[0]??{}); return {walletCurrency:String(r.wallet_currency),initialMinimumDeposit:Number(r.initial_minimum_deposit),commissionRate:Number(r.commission_rate),updatedAt:String(r.updated_at)} satisfies MarketplaceFinancialSettings; },
    async listIncidents(projectId,resolved) { const {data,error}=await getSupabaseClient().rpc("admin_list_marketplace_incidents",{target_project_id:projectId,target_resolved:resolved??null}); throwIfError(error); return ((data??[]) as Record<string,unknown>[]).map(r=>({jobId:String(r.job_id),serviceCode:String(r.service_code),incidentReason:String(r.incident_reason),incidentOpenedAt:String(r.incident_opened_at),driverDisplayName:r.driver_display_name?String(r.driver_display_name):null,resolved:Boolean(r.resolved),resolution:r.resolution?String(r.resolution):null} satisfies MarketplaceIncident)); },
