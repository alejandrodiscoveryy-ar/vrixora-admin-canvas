...
begin
  ...
  return query select now(),
    ...
    count(*) filter(where d.status='active' and d.suspended_at is null and not exists(select 1 from public.marketplace_work_trials t where t.project_id=d.project_id and t.user_id=d.user_id and t.started_at<=now() and now()<t.ends_at))::bigint,
...
begin
  ...
  select jsonb_build_object('job',to_jsonb(j)-'assigned_driver_user_id'-'assigned_vehicle_id','service_request',to_jsonb(sr)-'details',
    'assignment',to_jsonb(a),'billing_mode',a.billing_mode,'reservation_status',cr.status,
    'timeline',coalesce((select jsonb_agg(to_jsonb(e)-'metadata' || jsonb_build_object('metadata',case when e.action='incident_resolved' then e.metadata else '{}'::jsonb end) order by e.created_at) from public.job_events e where e.project_id=j.project_id and e.job_id=j.id),'[]'::jsonb),
    'incident_resolution',to_jsonb(ir),'customer',case when can_customers then jsonb_build_object('display_name',c.display_name,'whatsapp_phone',c.whatsapp_phone) else null end,
    'financial',case when can_pay then jsonb_build_object('reservation',to_jsonb(cr),'ledger_transaction_id',ir.ledger_transaction_id) else null end)
...
begin
 ...
 if target_resolution='completed' then if a.cancelled_at is not null then raise exception 'ASSIGNMENT_ALREADY_CANCELLED' using errcode='22023'; end if; update public.job_assignments set completed_at=coalesce(completed_at,now()) where project_id=target_project_id and job_id=j.id; else if a.completed_at is not null then raise exception 'ASSIGNMENT_ALREADY_COMPLETED' using errcode='22023'; end if; update public.job_assignments set cancelled_at=coalesce(cancelled_at,now()) where project_id=target_project_id and job_id=j.id; end if;
