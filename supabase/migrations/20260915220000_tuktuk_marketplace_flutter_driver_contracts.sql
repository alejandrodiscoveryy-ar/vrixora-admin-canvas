...
begin
 ...
 values(target_project_id,referrer,actor,upper(btrim(target_code)),'referral_link',false,actor,actor,0) returning id into result;
 return result;
end;
$$;
...
begin
 ...
 return coalesce((select jsonb_agg(jsonb_build_object('relationship_id',r.id,'name',coalesce(nullif(p.display_name,''),'Usuario'),'status',case when mr.id is not null or lt.legacy_reward_id is not null then 'rewarded' when r.qualified_at is not null then 'qualified' else 'registered' end,'reward_days',0,'reward_amount',coalesce(mr.reward_amount_snapshot,lt.reward_amount_snapshot),'reward_currency',coalesce(mr.reward_currency_snapshot,lt.reward_currency_snapshot),'reward_source',case when mr.id is not null then 'marketplace_referral_reward' when lt.legacy_reward_id is not null then 'legacy_referral_transition' else null end,'qualification_job_id',mr.qualification_job_id,'rewarded_at',coalesce(mr.qualified_at,lt.migrated_at),'created_at',r.created_at,'qualified_at',r.qualified_at) order by r.created_at desc) from public.referral_relationships r join public.profiles p on p.id=r.referred_user_id left join public.marketplace_referral_rewards mr on mr.project_id=r.project_id and mr.relationship_id=r.id left join public.marketplace_legacy_referral_reward_transitions lt on lt.project_id=r.project_id and lt.relationship_id=r.id where r.project_id=target_project_id and r.referrer_user_id=actor and not r.is_test),'[]'::jsonb);
end;
$$;
