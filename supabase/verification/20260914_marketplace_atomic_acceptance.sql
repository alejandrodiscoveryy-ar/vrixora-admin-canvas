...
begin
  ...
  foreach definition in array array['cargo_length_cm is null or cargo_length_cm >= 0','cargo_width_cm is null or cargo_width_cm >= 0','cargo_height_cm is null or cargo_height_cm >= 0'] loop
    if not exists(select 1 from pg_constraint where conrelid='public.vehicles'::regclass and contype='c' and pg_get_constraintdef(oid) like '%' || definition || '%') then raise exception 'TEST_FAILED: vehicle dimension nonnegative check missing for %',definition; end if;
  end loop;
  ...
  foreach relation_name in array array['public.accept_job(uuid,text,uuid)','public.advance_my_marketplace_job(uuid,text,uuid)','public.cancel_my_marketplace_job(uuid,text,uuid)'] loop
    ...
    if has_function_privilege('anon',relation_name,'EXECUTE') or not has_function_privilege('authenticated',relation_name,'EXECUTE') then raise exception 'TEST_FAILED: RPC privilege contract invalid for %',relation_name; end if;
  ...
  foreach definition in array array['auth.uid()', 'security definer', 'tuktuk-control', 'for update', 'has_active_marketplace_suite', 'has_confirmed_marketplace_initial_deposit', 'status <> ''published''', 'expires_at', 'is_active', 'is_available', 'marketplace_status=''active''', 'vehicle_services', 'passenger_count', 'cargo_weight_kg', 'cargo_volume_m3', 'cargo_length_cm', 'cargo_width_cm', 'cargo_height_cm', 'required_body_type', 'round(job_record.final_price * job_record.commission_rate_snapshot, 2)', 'available_balance < commission_amount', 'job_assignments', 'acceptance_idempotency_key=target_idempotency_key', 'commission_reservations r', 'status=''accepted''', 'action=''accept'''] loop
  ...
  if function_definition not like '%target_action is null%' or function_definition not like '%target_action not in (''start_en_route'',''mark_pickup'',''start_service'',''complete_service'')%' or function_definition like '%target_status%' or function_definition not like '%marketplace_job_transition_allowed%' or function_definition not like '%job_assignments%' or function_definition not like '%commission_reservations%' or function_definition not like '%job_commission%' or function_definition not like '%amount_delta%' or function_definition not like '%status=''consumed''%' or function_definition not like '%status=''settled''%' then raise exception 'TEST_FAILED: advance or settlement contract incomplete'; end if;
  if position('from public.job_assignments where project_id=tuktuk_project_id and job_id=job_record.id for update;' in function_definition)=0 or position('if not found then raise exception ''job_assignment_not_found''' in function_definition) < position('from public.job_assignments where project_id=tuktuk_project_id and job_id=job_record.id for update;' in function_definition) or position('if not found then raise exception ''marketplace_wallet_not_found''' in function_definition) < position('from public.wallets where project_id=tuktuk_project_id and user_id=actor for update;') or position('if not found then raise exception ''commission_reservation_not_found''' in function_definition) < position('from public.commission_reservations where project_id=tuktuk_project_id and job_id=job_record.id for update;') then raise exception 'TEST_FAILED: completion locks lack immediate existence checks'; end if;
  select lower(pg_get_functiondef('public.cancel_my_marketplace_job(uuid,text,uuid)'::regprocedure)) into function_definition;
  foreach definition in array array['cancelled_by_driver','status=''released''','release_reason','status=''incident''','incident_from_status=''in_progress''','open_incident','is_available=is_active','previous_status:=job_record.status','event_record.reason is not distinct from reason','marketplace_job_transition_allowed(''in_progress'',''incident'',''driver'')','job_assignment_not_found','marketplace_wallet_not_found','commission_reservation_not_found'] loop
    if position(definition in function_definition)=0 then raise exception 'TEST_FAILED: cancel contract lacks %',definition; end if;
  end loop;
  if function_definition like '%insert into public.wallet_transactions%' then raise exception 'TEST_FAILED: cancel must not debit ledger'; end if;
  if position('previous_status,''cancelled_by_driver''' in function_definition)=0 then raise exception 'TEST_FAILED: cancel event does not preserve previous status'; end if;
  if position('status=''released''' in function_definition) > position('elsif job_record.status=''in_progress''' in function_definition) then raise exception 'TEST_FAILED: incident branch releases reservation'; end if;
