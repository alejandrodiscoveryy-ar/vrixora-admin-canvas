...
begin
  ...
  if d not like '%tuktuk-control%' or position('phone ~ ''^\\+[1-9][0-9]{7,14}$''' in d)=0 or d not like '%owner_user_id=d.user_id%' or d not like '%owner_user_id=v.owner_user_id%' then raise exception 'TEST_FAILED: onboarding project, phone or photo ownership incomplete'; end if;
