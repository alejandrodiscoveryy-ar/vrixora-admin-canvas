...
begin
  ...
  if has_table_privilege('anon','public.marketplace_work_trials','select') or has_table_privilege('anon','public.marketplace_work_trials','insert') or has_table_privilege('anon','public.marketplace_work_trials','update') or has_table_privilege('anon','public.marketplace_work_trials','delete') or has_table_privilege('authenticated','public.marketplace_work_trials','select') or has_table_privilege('authenticated','public.marketplace_work_trials','insert') or has_table_privilege('authenticated','public.marketplace_work_trials','update') or has_table_privilege('authenticated','public.marketplace_work_trials','delete') then raise exception 'TEST_FAILED: direct trial access'; end if;
  ...
  if not exists(select 1 from pg_trigger where tgrelid='public.marketplace_work_trials'::regclass and tgname='audit_marketplace_work_trials') or has_function_privilege('anon','app_private.protect_marketplace_work_trial()','execute') or has_function_privilege('authenticated','app_private.protect_marketplace_work_trial()','execute') then raise exception 'TEST_FAILED: trial audit/protector incorrect'; end if;
  ...
  if d not like '%tuktuk-control%' or position('phone ~ ' || quote_literal('^' || chr(92) || '+[1-9][0-9]{7,14}$') in d)=0 or d not like '%owner_user_id=d.user_id%' or d not like '%owner_user_id=v.owner_user_id%' then raise exception 'TEST_FAILED: onboarding project, phone or photo ownership incomplete'; end if;
  ...
  if exists(select 1 from public.job_assignments where billing_mode is null or commission_amount_snapshot is null) then raise exception 'TEST_FAILED: LEGACY_JOB_ASSIGNMENT_WITHOUT_RESERVATION'; end if;
