...
begin
  ...
  foreach function_name in array array['public.marketplace_customer_sessions','public.customers','public.service_requests','public.jobs','public.job_events','public.job_assignments','public.commission_reservations','public.wallets','public.wallet_transactions'] loop
    if has_table_privilege('anon',function_name,'select') or has_table_privilege('authenticated',function_name,'select') then raise exception 'TEST_FAILED: direct access exposed for %',function_name; end if;
  end loop;
  ...
  if definition not like '%extensions.digest%' or definition not like '%^\\+[1-9][0-9]{7,14}$%' or definition not like '%idempotency_key%' or definition like '%raw_token%' then raise exception 'TEST_FAILED: start session privacy contract incomplete'; end if;
  ...
  if definition not like '%for update%' or definition not like '%wallet_commission%' or definition not like '%trial_free%' or definition not like '%customer_cancellation_requires_support%' or definition not like '%is_available=is_active%' then raise exception 'TEST_FAILED: customer cancellation incomplete'; end if;
...
-- invalid WhatsApp; concurrent session creation and idempotent retries; driver IDOR contact;
